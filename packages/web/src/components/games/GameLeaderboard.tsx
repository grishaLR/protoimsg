...
export function GameIcon({
  ...
  icon: keyof typeof GAME_ICON_PATHS;
  size?: number;
}) {
  return (
    <svg
...
export type LbEntry = { did: string; score: number };
...
export function StatsCol({ stats }: { stats: { fast: GameStats; faster: GameStats } }) {
...
export function LeaderboardCol({
  title,
  entries,
  viewerDid,
  handleMap,
    ...
    <div className={styles.lbCol}>
      ...
      {items.map((item) => (
        <div key={item.href} className={styles.lbEntry}>
          {/* eslint-disable-next-line no-restricted-syntax -- hrefs are hardcoded constants, not user input */}
