...
import { translateTexts, fetchTranslateStatus } from './api';
...
export function TranslationProvider({ children }: { children: ReactNode }) {
  ...
  const [autoTranslate, setAutoTranslateState] = useState(
    () => storage.getBoolean(AUTO_TRANSLATE_KEY) === true,
  );
  ...
  // Check availability once auth token is available
  useEffect(() => {
  ...
  const setAutoTranslate = useCallback((value: boolean) => {
    ...
    storage.set(AUTO_TRANSLATE_KEY, value);
  }, []);
  ...
  const requestBatchTranslation = useCallback(
    (texts: string[]) => {
      ...
      // Chain onto the queue so batches process sequentially
